...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	if(huart->Instance == USART2)
	{
		...
		if(bluetooth_ack_pending < 255)
		{
			bluetooth_ack_pending++;
		}
		
		// 从邮件队列内存池分配内存
		uint8_t (*data_copy)[2] = osMailAlloc(myMail01Handle, 0);
		if(data_copy != NULL)
		{
	...
	if(huart->Instance == USART3)
	{
		...
		{
			maixcam_cmd[maixcam_cmd_index++] = rx_byte;
			if(maixcam_cmd_index >= 2)
			{
    ...
    if(counter==11) //���յ�11������
    { 
			memcpy(Re_buf,Temp,11);   //��Temp���鸴�Ƶ�Re_buf
      counter=0;                //���¸�ֵ��׼����һ֡���ݵĽ���
